...
- 当前状态：M11 工程阶段，已完成 DSL Schema、Renderer、Figma 插件静态 UI、FastAPI 后端、插件上传链路、真实 PNG deterministic region fallback DSL、visual primitive contract harness、OCR/DSL patch harness、百度 PP-OCRv5 异步 OCR provider，以及低风险可见文字替换 harness。
...
- M11 当前可选接入百度 PP-OCRv5 异步 OCR，并在 `TEXT_REPLACEMENT_MODE=apply` 时只对浅色纯色背景的低风险文字做可见替换。AI/OCR 输出不能直接成为 DSL 权威，必须经过合同、决策和校验。
