...
- 当前状态：M10 工程阶段，已完成 DSL Schema、Renderer、Figma 插件静态 UI、FastAPI 后端、插件上传链路、真实 PNG deterministic region fallback DSL、visual primitive contract harness、OCR/DSL patch harness，以及百度 PP-OCRv5 异步 OCR provider。
...
- M10 当前可选接入百度 PP-OCRv5 异步 OCR，但仍不做可见文字替换，不把 AI/OCR 输出直接写入 DSL。OCR 和 visual primitives 只能先产生可验证 patch。
