...
- 当前状态：M9 工程阶段，已完成 DSL Schema、Renderer、Figma 插件静态 UI、FastAPI 后端、插件上传链路、真实 PNG deterministic region fallback DSL、visual primitive contract harness，以及 OCR/DSL patch harness。
...
- M9 当前不接真实 OCR，不做可见文字替换，不把 AI/OCR 输出直接写入 DSL。OCR 和 visual primitives 只能先产生可验证 patch。
