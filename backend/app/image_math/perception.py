...
def preprocess_png_for_yolo_like_model(data: bytes, *, input_size: int) -> tuple[np.ndarray[Any, Any], dict[str, float]]:
    with Image.open(BytesIO(data)) as image:
        return preprocess_image_for_yolo_like_model(image.convert("RGB"), input_size=input_size)


def preprocess_image_for_yolo_like_model(image: Any, *, input_size: int) -> tuple[np.ndarray[Any, Any], dict[str, float]]:
...
def rows_from_output(data: np.ndarray[Any, Any]) -> np.ndarray[Any, Any]:
...
def nms_candidates(candidates: list[dict[str, Any]], *, iou_threshold: float) -> list[dict[str, Any]]:
    ...
    for candidate in sorted(candidates, key=lambda item: item["score"], reverse=True):
        if all(bbox_iou(candidate["bbox"], kept_candidate["bbox"]) < iou_threshold for kept_candidate in kept):
