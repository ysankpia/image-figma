...
def get_task_icon_gap_candidates(task_id: str) -> dict[str, object]:
    ...
    result = state.database.get_icon_gap_candidate_result(task_id)
    if result is None:
        raise ApiError(
            "ICON_GAP_CANDIDATE_NOT_FOUND",
            "Icon gap candidate result not found.",
            status_code=status.HTTP_404_NOT_FOUND,
            stage="icon_gap_candidate_lookup",
            task_id=task_id,
        )

    gap_path = Path(result["gap_path"] or "")
    if not gap_path.exists():
        raise ApiError(
            "ICON_GAP_CANDIDATE_NOT_FOUND",
            "Icon gap candidate file not found.",
            status_code=status.HTTP_404_NOT_FOUND,
            stage="icon_gap_candidate_lookup",
            task_id=task_id,
        )
    ...
    data: dict[str, object] = {
        "taskId": task_id,
        "status": result["status"],
        "gapIcons": document.get("gapIcons", []),
        "blockedHints": document.get("blockedHints", []),
        "gapOverlay": document.get("gapOverlay"),
        "warnings": document.get("warnings", []),
        "meta": document.get("meta", {}),
    }
    if document.get("error"):
...
def get_task_icon_visible_fallback(task_id: str) -> dict[str, object]:
    ...
    result = state.database.get_icon_visible_fallback_result(task_id)
    if result is None:
        raise ApiError(
            "ICON_VISIBLE_FALLBACK_NOT_FOUND",
            "Icon visible fallback result not found.",
            status_code=status.HTTP_404_NOT_FOUND,
            stage="icon_visible_fallback_lookup",
            task_id=task_id,
        )

    fallback_path = Path(result["fallback_path"] or "")
    if not fallback_path.exists():
        raise ApiError(
            "ICON_VISIBLE_FALLBACK_NOT_FOUND",
            "Icon visible fallback file not found.",
            status_code=status.HTTP_404_NOT_FOUND,
            stage="icon_visible_fallback_lookup",
            task_id=task_id,
        )
    ...
    data: dict[str, object] = {
        "taskId": task_id,
        "status": result["status"],
        "visibleIcons": document.get("visibleIcons", []),
        "blockedPlacements": document.get("blockedPlacements", []),
        "visibleFallbackOverlay": document.get("visibleFallbackOverlay"),
        "warnings": document.get("warnings", []),
        "meta": document.get("meta", {}),
    }
    if document.get("error"):
...
def get_task_sam_visual_candidates(task_id: str) -> dict[str, object]:
    ...
    result = state.database.get_sam_visual_candidate_result(task_id)
    if result is None:
        raise ApiError(
            "SAM_VISUAL_CANDIDATE_NOT_FOUND",
            "SAM visual candidate result not found.",
            status_code=status.HTTP_404_NOT_FOUND,
            stage="sam_visual_candidate_lookup",
            task_id=task_id,
        )

    candidate_path = Path(result["candidate_path"] or "")
    if not candidate_path.exists():
        raise ApiError(
            "SAM_VISUAL_CANDIDATE_NOT_FOUND",
            "SAM visual candidate file not found.",
            status_code=status.HTTP_404_NOT_FOUND,
            stage="sam_visual_candidate_lookup",
            task_id=task_id,
        )
