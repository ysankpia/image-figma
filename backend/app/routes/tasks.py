...
@router.get("/tasks/{task_id}/m39-1-unit-structure-readiness")
def get_task_m39_1_unit_structure_readiness(task_id: str) -> dict[str, object]:
    task = state.database.get_task(task_id)
    if task is None:
        raise ApiError(
            "TASK_NOT_FOUND",
    ...
    report_path = state.settings.storage_root / "m30_1_uploads" / task_id / "m39_1" / "unit_structure_readiness_report.json"
    if not report_path.exists():
        raise ApiError(
            "M39_1_UNIT_STRUCTURE_READINESS_NOT_FOUND",
            "M39.1 unit structure readiness report not found.",
            status_code=status.HTTP_404_NOT_FOUND,
            stage="m39_1_unit_structure_readiness_lookup",
            task_id=task_id,
        )
    ...
    data: dict[str, object] = {
        ...
        "warnings": report.get("warnings", []),
        "modelSkippedReason": report.get("modelSkippedReason"),
        "candidateUnits": report.get("candidateUnits", []),
        "promotionHints": report.get("promotionHints", []),
        "outputReport": str(report_path),
        "stageTimings": timings,
    }
    return success_response(data)
