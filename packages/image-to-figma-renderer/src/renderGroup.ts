...
export async function renderGroup(context: RenderContext, element: DSLElement, path: string): Promise<FigmaNode> {
  const node = context.figma.createFrame();
  context.figma.setName(node, element.name ?? `Group / ${element.id}`);
