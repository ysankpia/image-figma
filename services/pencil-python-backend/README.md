...
独立的 Pencil `.pen` / `project.zip` / `selected-assets.zip` 交付服务。当前主产品入口是 assisted slice workspace：
...
`manual_slices.v1.json` 是最终交付真相源。PSD-like、M29、OCR、foreground audit 和模型证据只负责生成候选或 debug 信息。

服务仍保留自动 Pencil 项目导出路径，用于批量或诊断：
...
这会验证 `/api/pencil/slice-projects`：project creation、candidates、`manual_slices.v1.json`、
`review_state.v1.json`、export preview、`project.zip`、`selected-assets.zip`、selected asset counts
和 `.pen` visible image refs，并在 `OUT` 下写出 `acceptance_report.md` / `acceptance_report.json`。
...
Assisted slice review 是当前主产品路径，用于全自动切图不稳定的复杂页面：
...
工作台支持上传 1..N 张图、填写 `projectName`、选择 `boundarySource`、打开/关闭 `includeDebug`，创建成功后会自动跳转到 review workbench。旧的单项目上传页仍在 `/api/pencil/slice-projects/new`。
