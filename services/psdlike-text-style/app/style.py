...
from .colors import color_distance, color_hex, dominant_cluster_color, dominant_cluster_stats
from .schema import BBox, clamp_box
...
def infer_font_family(text: str) -> str:
    ...
    return "PingFang SC" if cjk > 0 else "Inter"
...
def estimate_text_background_for_box(rgb: np.ndarray, box: BBox) -> np.ndarray:
    ...
    padded = clamp_box(BBox(box.x - padding, box.y - padding, box.width + padding * 2, box.height + padding * 2), width, height)
    if padded is None:
