...
def normalize_bbox(value: dict[str, Any]) -> dict[str, int]:
    return {
        "x": int(round(float(value.get("x") or value.get("left") or value.get("xmin") or 0))),
        "y": int(round(float(value.get("y") or value.get("top") or value.get("ymin") or 0))),
        "width": int(round(float(value.get("width") or value.get("w") or 0))),
        "height": int(round(float(value.get("height") or value.get("h") or 0))),
    }
