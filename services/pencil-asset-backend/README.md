...
选择 tool + left drag selected slice   -> move selected slice
选择 tool + drag active resize handle  -> resize selected slice
选择 tool + right click canvas item    -> open candidate/selected context menu
...
candidates for the current page. Selected slices are independent delivery
assets: deleting a selected slice does not hide the candidate, and hiding a
candidate does not delete a selected slice. Frame colors are persisted under
`review_state.filters.colors`; they are review UI state only and do not affect
export. Use `V`, `B`, and `H` to switch to select, draw, and pan when focus is
not inside a form control.
