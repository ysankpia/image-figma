#!/usr/bin/env python3
"""Test unified vision with section_0001 BOTTOM HALF (47 items)."""
...
EVIDENCE_FILE = Path("/tmp/unified_vision_test/section_0001_bottom_evidence.json")

SECTION_BBOX = [27, 422, 611, 409]
...
def main():
    ...
    print(f"Model: {MODEL}")
    print(f"Section: section_0001 BOTTOM HALF {SECTION_BBOX}")
    ...
    try:
        ...
        print(f"Raw:\n{response_text[:2000]}")
        Path("/tmp/unified_vision_test/section_0001_bottom_raw.txt").write_text(response_text)
        return 1

    Path("/tmp/unified_vision_test/section_0001_bottom_result.json").write_text(
    ...
    for g in groups:
        ...
        for m in members:
            if m not in all_ids:
                errors += 1
                print(f"  ERROR: {m} not in evidence!")
            if m in grouped_ids:
                errors += 1
                print(f"  ERROR: {m} duplicate!")
            grouped_ids.add(m)
...
def post_json(url, payload):
    ...
    for attempt in range(3):
        try:
            ...
            if attempt < 2 and not (isinstance(e, urllib.error.HTTPError) and e.code < 500):
                ...
                raise RuntimeError(f"Provider failed: {e} {body}") from e
