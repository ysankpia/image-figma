...
- `TEXT_REPLACEMENT_MODE=apply` 只应用 `decision=accepted` 且 `quality.applyEligible=true` 的 replacement。
...
- apply 模式只应用 quality gate 通过的 replacement。
- accepted 但中高风险的 decision 保留报告并阻断 DSL 写入。
