...
- M14 fix 让 pill 和 bottom nav 使用 dominant local background，支持浅色背景上的彩色文字 `solid_light_background_colored_text`，并让 quality gate 读取 M14 strategy evidence，而不是只按 `hero/preview/tip` 粗略区域阻断。
...
- 本机首页样例存在时，用百度 OCR 的 38-block 回归基准验证 `男生`、`2026级新生`、`可视化选床...`、`预览选床界面`、`温馨提示`、提示正文和底部 `我的` 能进入 applied。
