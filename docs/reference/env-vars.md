...
`perception_model_report` 只记录模型候选框。`perception_source_compiler` 会把模型候选、OCR、source PNG pixels 和当前 M29.2 文档编译成增强版 M29.2 source ownership，然后交给后续 M29.3/M29.4/M29.5 正常处理。它不直接创建 DSL nodes，不授权 cleanup，不绕过 materializer。
