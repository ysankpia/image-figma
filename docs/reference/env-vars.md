...
| `SAM_VISUAL_CANDIDATE_MAX_IMAGE_EDGE` | M27 SAM2 推理前最长边缩放上限 | `1280` | 否 |
...
M27 SAM visual candidate filtering 由 `SAM_VISUAL_CANDIDATE_ENABLED` 控制，默认关闭，因为它需要本地 SAM2 runtime 和 checkpoint。开启后生成 `/sam-visual-candidates` 报告和 overlay，把 SAM2 automatic masks 过滤成 accepted/blocked visual candidates。M27 不修改 DSL，不追加 DSL meta，不裁新 icon asset，不生成透明 PNG，不改变 Figma 可见输出。当前本机 checkpoint 建议放在 `/Volumes/WorkDrive/Models/sam2/sam2.1_hiera_tiny.pt`，不要提交到 git。
