...
Local download responses stream the stored file with `Bun.file()`, set `Content-Length` and `Accept-Ranges: bytes`, and support single-range `Range` requests with `206 Partial Content`. Download requests do not enter the export job queue. If a user waits before a project package download starts, the delay is normally the first-time zip generation job, not the already-generated file transfer. The current zip materialization still builds a complete zip buffer before writing it to local storage; a future heavy-file pass should replace that with streaming zip generation or object storage.
