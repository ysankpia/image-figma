...
> Reference-only Codia reverse-engineering audit. It is not the current product specification on this branch. Current product work targets Slice Studio: saved SliceRecord boxes -> `assets.zip` + `project.zip/design.pen`. This document may inform eval/reference work only.
