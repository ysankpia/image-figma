...
1. **absolute pixel threshold drift**：M29.2、M29.6、transparent asset 和 selected marker 相关逻辑仍有多处绝对像素阈值。它们不是文案/品牌特化，但会在不同分辨率、不同 OCR 字高、不同密度 UI 下表现成隐性特化。
2. **fixed candidate cap / scan budget**：M29.6 的 component/window candidate 提取仍存在固定 top-N 或固定窗口预算风险，可能让 table marker、status dot、selected marker 等小对象被大 foreground 挤掉。
...
2. 增加 scale-aware threshold profile，优先处理高风险绝对像素阈值。
3. 修复 M29.6 fixed top-N / fixed window cap 对小对象的漏检。
...
- 覆盖 1x/2x/3x scale synthetic UI，确保角色路径不被绝对像素阈值改变。
- 覆盖 M29.6 多小对象 component extraction，不因固定 top-N cap 丢失 marker。
