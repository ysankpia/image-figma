...
2. **fixed candidate cap / scan budget**：M29.6 的 component/window candidate 提取曾存在固定 top-N 或固定窗口预算风险。Stage 2 已把 `[:6]` component 截断替换为基于窗口面积和 scale 的 return budget，并把 generic scan/candidate budget 改为 area-density aware；Stage 3 已给 selected marker / status dot / table marker 增加 M29.6 report-only 正向 role evidence。后续 Stage 5 仍需把这些 role 接入 source promotion。
...
- 继续扩展 1x/2x/3x scale synthetic UI，覆盖 table marker / status dot / selected marker 从 M29.6 role 到 promotion/replay as shape 的完整路径。
- 覆盖 M29.6 marker role 后的 evidence/promotion correctness，不能只证明候选存在，还要证明后续不误分 icon/shape。
