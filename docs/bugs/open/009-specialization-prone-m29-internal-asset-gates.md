...
2. **fixed candidate cap / scan budget**：M29.6 的 component/window candidate 提取曾存在固定 top-N 或固定窗口预算风险。Stage 2 已把 `[:6]` component 截断替换为基于窗口面积和 scale 的 return budget，并把 generic scan/candidate budget 改为 area-density aware；Stage 3 仍需继续补 table marker/status dot/selected marker 的正向 role evidence。
3. **transparent gate coupling**：`analysis allowed`、`asset generated`、`visible replay allowed`、`cleanup allowed` 目前仍耦合得太紧。alpha 分析失败或未执行会直接让 evidence/promotion 缺少路径。
4. **promotion role narrowing**：internal source promotion 当前主路径仍以 `internal_icon_candidate -> raster_icon` 为中心，缺少 `selected_marker`、`table_marker`、`status_dot`、`internal_control_background` 这类正向 source role。
...
3. 修复 M29.6 fixed top-N / fixed window cap 对小对象的漏检。已完成 Stage 2 第一轮：connected component 固定 top 6 截断已移除，generic scan/candidate budget 改为 scale-aware area-density budget；Stage 3 继续处理 marker role correctness。
4. 拆分 transparent asset gate，允许 medium + strong independent evidence 的 candidate 进入 alpha analysis，但不绕过 evidence contract。
5. 增加 selected marker、table marker、status dot、internal control background 的正向 promotion role。
...
- 继续扩展 1x/2x/3x scale synthetic UI，覆盖 table marker / status dot / selected marker 的完整 role path。
- 覆盖 M29.6 多小对象 component extraction 后的 role correctness，不能只证明候选存在，还要证明后续 evidence/promotion 不误分 icon/shape。
- 覆盖 transparent asset 的 `analysisAllowed` 与 `visibleReplayEligible` 分离。
- 覆盖 selected marker / table marker / status dot replay as shape，而不是 replay as icon。
