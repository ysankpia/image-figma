...
本记录描述的是旧 Go Codia Beta / Codia-like tree 路径的质量债。该问题先被 Editable Draft Layer Pipeline 取代；当前分支可交付主线进一步切到 Slice Studio。Codia Beta 质量债不再作为 open product bug 跟踪；其中有价值的事实只能作为候选/eval/debug 背景，不能覆盖当前 saved SliceRecord truth。
