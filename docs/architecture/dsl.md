...
## M30 Evidence-Grounded Materialized DSL Variant

M30 generates a new DSL v0.1 variant instead of overwriting the base DSL. It is the first controlled bridge from M29 trusted evidence into Renderer-visible nodes:

```text
M29.0.5 textMembers -> DSL text
M29.0.5 safe shapeCandidates -> DSL shape
M29.0.5 safe visualAssets -> DSL image
```

M30 has two input modes:

```text
augment-existing-dsl:
  preserve the provided DSL and append M30 nodes
...
M29.1.3 and M29.0.3.2 audit-only outputs may be referenced in report/meta, but must not become visible DSL children.
