...
M15 增加 text-primitive binding harness：后端把 M14 可用 text candidate 绑定到现有 visual primitives 或 M15 推断出的 UI containers，写入 `backend/storage/text_bindings/{taskId}.json` 并通过 `/api/tasks/{taskId}/text-bindings` 暴露。默认 fake visual primitive provider 只有 fallback regions，因此 M15 会生成 `inferred_from_text_cluster` 容器，例如 badge、status badge、primary button、shortcut card、preview card、legend group、tip card 和 bottom nav item。推断容器只存在于 binding report，不回写 visual primitives，也不改变 Figma 可见输出。
