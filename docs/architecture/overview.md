...
- Layer and asset harness：M18 生成 layer separation candidates，M19 生成本地 asset slice candidates，M20 生成 icon crop candidates，M21 生成 icon coverage audit，M22 生成 region-guided icon gap candidates，M23 生成 icon placement plan；这些旁路合同都不改变 Figma 可见输出。
...
- Processing Pipeline：预处理、裁切、visual primitive candidates、OCR candidates、DSL patch、text replacement、binding、structure、annotation、layer separation、asset slice candidate、icon candidate、icon coverage audit、icon gap candidate、icon placement plan、DSL Builder、校验和基础修复。
- Storage：原图、资产、DSL 结果、primitive 结果、OCR 结果、patch 结果、text replacement 结果、binding/structure/annotation/layer separation/asset slice/icon/icon coverage/icon gap/icon placement 报告、日志。
