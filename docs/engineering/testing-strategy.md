...
- M29.1.1 pre-OCR symbol lineage audit 必须覆盖 eligible blocked not grouped、M29/M29.1 lineage 后续被 M29.0.3 demote 为 `text_noise`、后续 M29.0.7 suppression、text-like glyph sequence 不生成 surviving lineage、mixed conflict 可审计、finding bbox 只来自既有 evidence、overlay PNG 源图同尺寸且不生成 formal visual asset。
- M29.1.2 symbol lineage survival contract 必须覆盖 accepted group 输出 strong/medium lineage、uncertain group 输出 weak/medium lineage、eligible blocked candidate 输出 weak lineage、rejected text-like sequence 不输出 surviving lineage、M29.0.3 high overlap 无 lineage 仍是 `text_noise`、有 lineage 变 `mixed_symbol_text_candidate`、rejected lineage 仍是 `text_noise`、M29.0.7 对 mixed candidate audit-only。
- M29.1.3 mixed symbol/text conflict classification audit 必须覆盖 primary universe 只处理 `mixed_symbol_text_candidate`、非 mixed item 被忽略、unknown 默认 `keep_mixed_symbol_text_conflict`、full OCR coverage/glyph sequence/text-like finding 进入 `text_owned_rejected_lineage`、strong/medium lineage + compact + partial OCR + non-glyph signals 进入 `future_promotable_uncertain_symbol_candidate`、冲突信号保持 keep mixed、所有 downstream permission flags 恒为 false、example crop 只是 audit evidence 且 overlay 源图同尺寸、不新增 bbox、不生成 formal visual asset、不修改 M29.0.3/M29.0.7/M29.0.4/M29.0.5、batch summary 包含分类总量和 example counts、JSON/Markdown/reason/risk/classification 不包含特化或恢复语义词。
- M29.0.3 visual evidence normalization 必须覆盖每个 M29.0.2 mediaEvidence item 恰好生成一个 VisualEvidenceItem、所有 item 都从原图裁 asset、accepted image/media candidate/icon candidate/text noise 分桶、text noise 保留但排后、overlay/preview PNG 可读、document validation 拒绝坏 bbox/重复 id/缺失 asset。M29.0.3.1 回归必须覆盖未传 M29.1 lineage JSON 时 baseline 不变、high OCR overlap 无 lineage 仍是 `text_noise`、surviving lineage + full OCR coverage/text-like token/text-like aspect/glyph sequence risk/weak eligible-blocked high overlap 变 `text_noise` 且带 `text_owned_rejected_lineage` sourceLineage、medium/strong lineage + partial OCR + non-glyph compact evidence 仍是 `mixed_symbol_text_candidate`、rejected lineage 不生成 formal visual asset。
- M29.0.3.2 residual mixed boundary review 必须覆盖 primary universe 只处理 M29.0.3 剩余 `mixed_symbol_text_candidate`、非 mixed item 被忽略、缺少 M29.1.3 conflict 进入 `insufficient_evidence`、M29.1.3 text rejected + M29.0.3-readable text signals 进入 `m2903_tightening_candidate`、M29.1.3 future + high text-like risk 进入 `m2913_classification_adjustment_candidate`、M29.1.3 future + compact/partial/not-glyph/repeated or label relation 进入 `candidate_for_future_uncertain_review`、conflicting/unknown signals 默认 `keep_residual_mixed_conflict`、所有 permission flags 恒为 false、example crop 只是 audit evidence、不新增 bbox、不生成 formal visual asset、review sheet 可读且源图同尺寸、forbidden terms check 生效、batch summary 包含 image-level/total counts 且 partial image failure 不会中断整个 batch。
...
- M29.0.6 member boundary quality audit 必须覆盖只从 M29.0.5 结果生成 finding、raw/dedup count 分离、weak text_noise member/object/batch dominance、duplicate source/member topology、exact/perceptual/conflicting asset duplicate、success baseline、top-K example 限制、overlay/preview 可读，且不新增 bbox/formal asset、不修改 M29.0.2/0.3/0.4/0.5。
