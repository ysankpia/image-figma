...
-> M29 ownership conservation / hierarchy / sibling / layout / auto-layout reports
-> M29 plan-driven materializer
-> M29 perception fate trace
-> DSL v0.1
-> Renderer
```

M30.2.2 已删除 pre-M29 legacy upload chain。M29 backend downstream pruning 已删除 M31-M39/M39.1 和 legacy ONNX proposer runtime。本阶段已下线 M29 Direct compare 产品入口、legacy M30 materialization 产品路径，以及默认 upload-preview 中旧的 M29.6 -> transparent asset -> evidence contract -> internal source promotion -> promoted rerun loop。测试不再要求这些旧 routes、env、modules 或 reports 存在。
...
- perception model 默认开启，perception source compiler 只在 M29.2 ownership 边界增强 source objects。
- `UPLOAD_PREVIEW_RUNTIME_MODE=interactive` 不运行旧 M29.6/promotion loop；`diagnostic` / `full` 只添加 post-materialization diagnostics。
...
uv run pytest \
  ...
  tests/test_m29_perception_fate_trace.py \
...
- default completed task writes perception model, perception source compiler, and perception fate trace artifacts when `M29_PERCEPTION_MODEL_ENABLED=true`.
- default completed task does not write M29.6 media internal decomposition, transparent asset, evidence contract, internal source promotion, bridge fate, or promoted rerun artifacts.
- diagnostic/full runtime writes design token, B-stage quality, and DSL visual comparison reports without changing final DSL.
...
- M29.2 media regions remain fallback/residual owners; model-derived foreground source objects must still compile through M29.2 before M29.5 replay.
- legacy M29.6, transparent asset, evidence contract, internal source promotion, and bridge fate packages remain testable as compatibility modules when explicitly targeted, but they are not default upload-preview stages.
...
- copied image asset cleanup is executed only with M29.5 copied image asset cleanup targets.
