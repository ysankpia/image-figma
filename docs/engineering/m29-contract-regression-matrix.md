...
| M29-CR-033 | Direct Replay | M29.5 plan items | 只 materialize accepted visible actions | suppressed/preserve/diagnostic 仍可见 | covered | `test_m29_direct_replay.py::test_m295_replay_plan_controls_visible_nodes` |
| M29-CR-034 | Direct Replay | plan cleanup targets | copied image asset cleanup 仅由 plan 授权 | 关系不明也擦 copied image asset | covered | `test_m29_direct_replay.py::test_m295_plan_cleanup_targets_drive_copied_image_asset_erasure` |
| M29-CR-035 | Direct Replay | plan 无 copied image cleanup target | copied image asset 不擦 | fallback cleanup 权限扩散到 copied asset | covered | `test_m29_direct_replay.py::test_m295_plan_without_asset_cleanup_target_does_not_erase_copied_image` |
| M29-CR-036 | Direct Replay | raw geometry radius | 只有 geometry fit radius 可写 DSL radius | bbox-derived radius 被当 style truth | covered | `test_m29_direct_replay.py::test_m295_shape_replay_preserves_geometry_fit_radius` |
| M29-CR-037 | Direct Replay | preserve raster text | 不创建 visible node，不擦 fallback | 不可编辑 text 被擦掉 | covered | `test_m29_direct_replay.py::test_m292_preserve_raster_text_is_not_erased_from_fallback` |
| M29-CR-038 | Upload Pipeline | M29.3.1/M29.4/M29.5 stages | report/plan stages不改变 visible DSL | branch experiment 污染主线 `/dsl` | covered | `test_m30_upload_pipeline.py::test_upload_m30_preview_uses_production_artifact_profile_by_default` |
| M29-CR-042 | Owner Boundary / Materialization | raw `text_support_background` 进入 M29.0.2/M29.0.3/M29.0.7/M29.0.5 | source subtype/reasons 保留，ownership 为 `shape_owned`，shape candidate 仍为 `candidate` | support shape 被 high text overlap 当成 text noise 或 ordinary unsafe shape | covered | `test_text_masked_media_audit.py::test_source_support_shape_enters_media_evidence_with_lineage_reason`, `test_visual_evidence_normalization.py::test_source_support_shape_survives_high_text_overlap`, `test_text_visual_ownership_gate.py::test_source_support_shape_with_text_overlap_remains_shape_owned`, `test_text_aware_visual_object_refinement.py::test_text_support_background_shape_candidate_preserves_source_contract` |
| M29-CR-043 | Materialization / Direct Replay | source-proven support shape 与 text member 重叠 | M30 support shape materializes below text；M29 Direct consumes raw fill/radius | 全局放宽 ordinary shape/text overlap 或从 text bbox 伪造背景 | covered | `test_evidence_grounded_dsl_materialization.py::test_source_proven_text_support_shape_materializes_below_text`, `test_evidence_grounded_dsl_materialization.py::test_ordinary_overlapping_shape_candidate_remains_unsafe`, `test_m29_direct_replay.py::test_m295_text_support_background_shape_uses_geometry_fit_radius` |
...
现有 pytest 是否覆盖？
如果没有覆盖，是否先补 test？
是否仍保持 M29.4 report-only、M29.5 plan-only、M29 Direct branch-only？
```
