...
| M29-CR-051 | Replay Plan / Ownership Conservation | 同类 `icon_replay/icon_replay` 或 `shape_replay/shape_replay` 明显重叠 | M29.5 将低优先级同 owner visible replay item 改为 `suppress_duplicate`，并记录 `visibleOverlapSuppressedCount` | 同一 foreground/support owner 被 materializer 画两遍 | covered | `test_m29_replay_plan.py::test_m295_suppresses_same_icon_contained_visible_overlap`, `test_m29_replay_plan.py::test_m295_suppresses_same_shape_high_overlap_without_suppressing_text_background` |
